...
class IngestOrchestrator:
    ...
    async def run_all(self) -> list[IngestResult]:
        """Process all active sources concurrently.

        Each source gets its own isolated AsyncSession, since a single
        AsyncSession cannot be safely shared across concurrent coroutines
        (doing so causes sqlalchemy.exc.MissingGreenlet errors). Concurrency
        is bounded by a semaphore so we don't exceed the DB connection
        pool's capacity (see app/core/database.py — default pool_size=5,
        max_overflow=10 => 15 connections max).
        """
        sources = await self._rss_repo.list_active()
        logger.info(
            "Starting concurrent ingest for %s active feeds (concurrency=%s)",
            len(sources),
            self._concurrency,
        )
        ...
        async def _run_isolated(source: RssSource) -> IngestResult:
            async with semaphore:
                async with AsyncSessionLocal() as source_session:
                    isolated_orchestrator = IngestOrchestrator(
                        source_session, concurrency=self._concurrency
                    )
                    return await isolated_orchestrator.run_source(source)

        results = await asyncio.gather(*[_run_isolated(source) for source in sources])
        logger.info("Completed concurrent ingest for %s feeds", len(results))
        return list(results)
...
#             except Exception as exc:
#                 logger.exception("Fatal unhandled exception running source %s (URL: %s)", source.source, source.feed_url)
...
#             except Exception as exc:
#                 await self._session.rollback()
#                 logger.exception("Silent failure: Failed to process article entry (URL: %s)", article_url)
#                 self._record_error(
#                     result,
#                     stage="parse",
#                     code="ARTICLE_PROCESSING_FAILED",
